...
Full layout and interaction specification for the **Versions + Expansions** tab on a Collection repository page. This tab is the primary surface for managing version history, expansion state, refresh status, and locking.
...
| Needs Refresh indicator | fixed 30px | Yellow ⚠ dot if any expansions need a refresh |
...
- Needs Refresh indicator shown if any expansion for this version needs a refresh
...
- Tooltip on hover: "One or more expansions for this version need a refresh. Select this version and rebuild."
- Clicking the indicator selects the version and scrolls the right panel to the expansion that needs a refresh
...
| Needs Refresh badge | fixed 60px | "Needs Refresh" pill (yellow) if expansion needs a refresh |
...
**Needs Refresh**
- Yellow "Needs Refresh" pill in the refresh status column
- Tooltip: "Built before [Source] released [versionID] on [date]. Rebuild to pick up changes."
- Row is still browsable; content is not blocked from use
...
| Action | Available When | Permission |
|---|---|---|
| Browse expansion | Released / Needs Refresh | Read |
...
### "Needs Refresh" Badge Interaction

- Needs Refresh badge is a clickable chip
...
- "Dismiss" removes the Needs Refresh flag for this expansion only (without rebuilding); requires owner permission; logged in audit trail
...
- The expansion that needs a refresh is scrolled into view in the right panel
